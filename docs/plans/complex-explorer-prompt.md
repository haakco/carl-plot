...
Typography: system-ui, 12–18px scale, weights 400/500/600
  - Data/coordinates: monospace with tabular-nums
  - Labels/UI: 13px, 500 weight
...
- **Width**: 200px fixed, collapsible to icon-only (48px) on mobile
...
|---------|---------|-------|-------|
| Domain coloring canvas | Skeleton gradient | Gray plane with grid (no function) | "WebGL not supported" with fallback message |
| Pole/zero list | N/A | "Drag a pole or zero onto the plane to begin" with arrow pointing to toolbox | N/A |
| Formula bar | "f(z) = ..." placeholder | "f(z) = 1" (identity, no poles/zeros) | Red-highlighted parse error with message |
| 3D surface view | Wireframe loading skeleton | Flat plane at z=1 | "WebGL context lost — click to restore" |
| Expression input | N/A | Placeholder: "Type an expression, e.g. z^2 + 1/z" | Inline error below input with the compilation error |
...
├── tailwind.config.ts                 # Tailwind 4 tokens
└── package.json
```
...
1. Scaffold project: Vite + Vike SPA + React 19 + Tailwind 4 + shadcn/ui init
